...
S_to_I <- function(I_index,
                   ...
                   contact_list,
                   population # list of possible contacts for each unit (with self removed)
) {
  ...
  poss_contacts <- contact_list[effc_id]
  ...
  if (length(effc) > 0) {
    ...
    contact_ids <- effcIdGen2(poss_contacts, effc, N, population)  # effcIdGenerator produces a list of effectively contacted unit ids (stored in contact_ids vector) using random sampling of population IDs (see effcIdGenerator code)
