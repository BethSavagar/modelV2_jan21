...
state_update <- function(
  ...
  ############################################################################
  ############# EXPOSURE AND TRANSMISSION POTENIAL ##################
  ...
  if(heterogeneity == "fixed"){
    ...
    if(correlation == "pos"){ # if exposure is positively correlated with transmission
      ...
      fixed_exp <- vector(length = length(fixed_effc)) # vector for exposure potential
      pExp <- N:1 # vector of exposure potentials ordered from high to low (NB: pExp could be fixed_exp[rev(order(fixed_effc))] )
      fixed_exp[order(fixed_effc)] <- pExp # assign pExp vector to index in order of fixed_effc vector
      ...
      ## code below suggested by GF did not work:
    ...
    fixed_effc <- rep(R0_estimate, N) # all units transmission potential == R0
    fixed_exp <- rep(1, N) # all units exposure potential == 1
    
  }else if(heterogeneity == "variable"){
    fixed_effc <- vector() # empty vector will be populated every 'generation'
    fixed_exp <- vector() # empty vector will be populated every 'generation'
  }
