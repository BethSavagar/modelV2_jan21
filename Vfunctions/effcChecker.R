effcChecker <- function(effc_raw, # raw list of effective contacts per unit
                        N, # population size
                        k_estimate, # dispersion parameter for transmission
                        R0_estimate # R0
                        ) {
  ...
  if (length(effc_N_id) > 0) {
    for (i in effc_N_id) {
      ...
      effc_i <- effc_raw[i] # assign variable effc_i to the number of effective contacts for the indexed unit (position i in effc_num vector)
      ...
      while (effc_i >= N) {
        new_i <- rnbinom(1, # simulate for single infected unit
                         ...
                         mu = R0_estimate) # average number of effective contacts, as above
        effc_i <- new_i # update effc_i within while loop
      }
      
      effc_raw[i] <- effc_i # Update the effc_num vector with the new number of effective contacts for the infected unit.
    }
  }
  return(effc_raw)
}
