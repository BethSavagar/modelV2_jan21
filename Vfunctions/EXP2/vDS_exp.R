  ...
  if(length(V_timing) > 0 & V_prop > 0){
    ...
    if(any(gen == V_timing)){ # if the current generation is in the V_timing vector (vac schedule)
      ...
      if(V_mode == "target" & heterogeneity == "fixed"){
        ...
        newV_index <- (1:N)[rev(order(effc_dist))][1:round(V_prop*N)] # subset population for vac by reverse order of NBD, subset proportion corresponding to V_prop
      ...
      if(V_mode == "non_target" & heterogeneity == "fixed"){
        ...
        newV_index <- (1:N)[order(effc_dist)][1:round(V_prop*N)]
